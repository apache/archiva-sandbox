    ...
    extends AbstractDavTestCase
{
        ...
        throws Exception
    {
        ...
        // Execute Test
        URI testURI = davServer.baseURI.resolve( "quotes/franklin/benjamin" );
        ...
        throws Exception
    {
        ...
        {
            URI testURI = davServer.baseURI.resolve( path );
            MkColMethod method = new MkColMethod( testURI );
            int status = client.executeMethod( method );
            // Verify Results
            assertEquals( "Status (" + path + ")", HttpStatus.SC_CREATED, status );
            assertTrue( "Created Directory (" + path + ")", new File( serverRoot, path ).exists() );
        }
    }
        ...
        throws Exception
    {
        ...
        // Execute Test
        URI testURI = davServer.baseURI.resolve( "quotes" );
        MkColMethod method = new MkColMethod( testURI );
        HttpClient client = new HttpClient();
        int status = client.executeMethod( method );
